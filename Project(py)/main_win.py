...
class Main_Win(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.img = QImage('../Project/img_1.png')
        self.pixmap = QPixmap(self.img)
        self.image.setPixmap(self.pixmap)
        self.radio_button = '0'
        # Создаем группу кнопок и добавляем туда кнопки
    ...
    def doo(self):
        for i in self.radio_button_group.buttons():
            # Идём по группе кнопок и проверяем какую из них нажали
            if i.isChecked():
                self.radio_button = i.text()[-2]
                # Записываем в переменную номер картинки
        # Проверяем была ли нажата какая-то кнопка и открытие окна
        if self.radio_button == '1':
            ...
            # Открытие окна предупреждения, что вы не выбрал тип графика если не было нажатой кнопки
